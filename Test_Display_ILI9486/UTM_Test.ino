...
void loop() {
 ...
 while (digitalRead(clockpin_load)==HIGH) {} //if clock is LOW wait until it turns to HIGH
 tempmicros=micros();
 while (digitalRead(clockpin_load)==LOW) {} //wait for the end of the HIGH pulse
 if ((micros()-tempmicros)>500){ //if the HIGH pulse was longer than 500 micros we are at the start of a new bit sequence
   decode1(); //decode1 the bit sequence
  }

 while (digitalRead(clockpin_displacement)==HIGH) {} //if clock is LOW wait until it turns to HIGH
 tempmicros=micros();
 while (digitalRead(clockpin_displacement)==LOW) {} //wait for the end of the HIGH pulse
 if ((micros()-tempmicros)>500){ //if the HIGH pulse was longer than 500 micros we are at the start of a new bit sequence
...
void decode1() {
  ...
  value=0;
  for (i=0;i<23;i++) {
    while (digitalRead(clockpin_load)==HIGH) { } //wait until clock returns to HIGH- the first bit is not needed
    while (digitalRead(clockpin_load)==LOW) {} //wait until clock returns to LOW
...
void decode2() {
  ...
  value=0;
  for (i=0;i<23;i++) {
    while (digitalRead(clockpin_displacement)==HIGH) { } //wait until clock returns to HIGH- the first bit is not needed
    while (digitalRead(clockpin_displacement)==LOW) {} //wait until clock returns to LOW
